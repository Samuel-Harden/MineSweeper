	...
	while (userChoice != true)
	{
		...
		case '1':
		{
			...
			if (mineCheck == true)
			{
				...
				// Print out the game Time in Seconds
				final = clock() - init;
				std::cout << std::endl;
				std::cout << " This Game Has Lasted " << (double)final / ((double)CLOCKS_PER_SEC)
					<< " Seconds" << std::endl;
...
void MineSweeper::fillGridWithGuess(int userInputX, int userInputY)
{
	...
	if (((userInputX) >= 0) && ((userInputX) < columns) && ((userInputY) >= 0) && ((userInputY) < rows))
	{
		...
		if (mineGrid->getChar(userInputX, userInputY) != 'm')
		{
			...
			//Function Call ONLY HERE FOR TESTING 
			//playerGrid->printGrid();
			//system("CLS");

			if (mineGrid->getChar(userInputX, userInputY) == '.')
			{
				//Checks the Tile UP of user input
				if (((userInputY - 1) >= 0) && (playerGrid->getChar(userInputX, (userInputY - 1)) == '*'))
				{
					fillGridWithGuess(userInputX, (userInputY - 1));
				}

				//Checks the Tile to the LEFT of user input
				if (((userInputX - 1) >= 0) && (playerGrid->getChar((userInputX - 1), userInputY) == '*'))
				{
					fillGridWithGuess((userInputX - 1), userInputY);
				}

				//Checks the Tile DOWN of user input
				if (((userInputY + 1) < rows) && (playerGrid->getChar(userInputX, (userInputY + 1)) == '*'))
				{
				...
				if (((userInputX + 1) < columns) && (playerGrid->getChar((userInputX + 1), userInputY) == '*'))
				{
					fillGridWithGuess((userInputX + 1), userInputY);
				}

				//Checks the Tile to the LEFT and UP of user input
				if (((userInputX - 1) >= 0) && ((userInputY - 1) >= 0) && (playerGrid->getChar((userInputX - 1), (userInputY - 1)) == '*'))
				{
					fillGridWithGuess(userInputX - 1, userInputY - 1);
				}

				//Checks the Tile to the RIGHT and UP of user input
				if (((userInputX + 1) < columns) && ((userInputY - 1) >= 0) && (playerGrid->getChar((userInputX + 1), (userInputY - 1)) == '*'))
				{
					fillGridWithGuess(userInputX + 1, userInputY - 1);
				}
				//Checks the Tile to the LEFT and DOWN of user input
				if (((userInputX - 1) >= 0) && ((userInputY + 1) < rows) && (playerGrid->getChar((userInputX - 1), (userInputY + 1)) == '*'))
				{
					fillGridWithGuess(userInputX - 1, userInputY + 1);
				}

				//Checks the Tile to the RIGHT and DOWN of user input
				if (((userInputX + 1) < columns) && ((userInputY + 1) < rows) && (playerGrid->getChar((userInputX + 1), (userInputY + 1)) == '*'))
				{
					fillGridWithGuess(userInputX + 1, userInputY + 1);
				}
			}
